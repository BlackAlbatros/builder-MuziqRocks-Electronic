...
export default function WatchPage() {
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      if (mediaKeys.includes(code) || ["MediaPlayPause", "MediaPause", "MediaPlay"].includes(e.key) || ["Enter", "OK", "Select", " "].includes(e.key)) {
        e.preventDefault();
        if (vid) {
  ...
  if (!videoId) {
    return (
      <div className="container mx-auto px-4 py-6 space-y-4">
        ...
        <Link to="/" className="inline-flex items-center gap-2 rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground">← Back to home</Link>
      </div>
    );
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-black">
      {source ? (
        <>
          <video ref={videoRef} key={video.id} controls autoPlay playsInline poster={video.thumbnail ?? undefined} className="h-full w-full" src={source}>
            Your browser does not support HTML5 video.
          </video>

          {showHome && (
            <div className="absolute left-4 bottom-20 z-60">
              <Link to="/" ref={homeLinkRef} data-home-link tabIndex={0} className="rounded-md bg-white/20 px-5 py-2 text-sm font-medium text-white hover:bg-white/30 focus:outline-none focus:ring-2 focus:ring-white">Home</Link>
            </div>
          )}
        </>
      ) : (
        <div className="flex flex-col items-center justify-center space-y-4">
          ...
          <Link to="/" className="inline-flex items-center gap-2 rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground">Home</Link>
        </div>
      )}
    </div>
  );
